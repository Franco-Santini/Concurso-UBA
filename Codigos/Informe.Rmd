...
plot_usmap(data = plot_data, values = "ganancias", regions = "states") +
  scale_fill_continuous(
    low = "white", high = "#A61C3C", name = "Ganancia (en $)"
  ) +
  theme(legend.position = "right",
        plot.title = element_text(hjust = 0.5, size = 14)) +
...
- $Y$ es el vector de variable respuesta que se distribuye $Poisson(\lambda)$
...
precios_optimizados_reg_poisson <- read.csv("../Predicciones/precios_optimizados_poisson.csv")
...
```{r tabla gbm}
# Lectura de los datos
precios_optimizados_gbm <- read.csv("../Predicciones/precios_optimizados_gbm.csv")
...
Luego a la hora de pensar estrategias para la maximización de las ganancias en base a la fluctuación en el precio de los productos, pudimos identificar las siguientes cuestiones de interés:
