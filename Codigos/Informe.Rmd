...
<img src="https://levainvierte.com/wp-content/uploads/2025/03/%C2%BFCuanto-es-lo-maximo-que-se-puede-invertir-en-CETES-Todo-lo-que-necesitas-saber-para-invertir-con-exito.jpg" width="600"/>
...
opt_res$scoreSummary |>
  ...
  row_spec(0, bold = TRUE, color = "white", background = "steelblue") |>  # encabezado
...
Para comenzar a entender que factores influyen en las ganancias producidas por los distintos productos, vamos a mostrar cuales son los productos más vendidos a través de los 3 años de estudio.
...
plot_ly(
  ...
  layout(
    title = "Ganancias promedio ($) por unidad",
    xaxis = list(title = "Producto",
      tickfont = list(size = 9)),
...
También resulta de interes evidenciar como evolucionó la ganancia que generaron los distintos productos a través del tiempo, en este caso nos centramos en los 5 productos que más ganancias generaron y visualizamos sus ganancias mensuales durante los 3 años de estudio.
...
scale_fill_continuous(
low = "white", high = "steelblue", name = "Ganancia total por estado"
) +
theme(legend.position = "right")

```

En el gráfico anterior podemos ver que los estados pertenecientes a la costa este de los Estados Unidos son los que producen mayor cantidad de ganancias, pero como comentamos anteriormente esto va a estar muy influenciado por la cantidad de tiendas presentes en cada estado. Para tener un panorama más real en relación a las diferencias en las ganancias de cada estado, vamos a realizar el mismo gráfico pero calculando la ganancia promedio por tienda dentro de cada estado.
...
read_csv("Datos/plot_data_1.csv")
...
Se observa que a pesar de que el percentil de precios más cercanos al costo son los que más cantidad de ventas producieron, pero no necesariamente se condice con el percentil que más ganancias generaron para la empresa.

Vemos que el segundo percentil en relación al aumento de los precios en función del costo es el que generó más ganancias, es decir, los productos que generaron más ganancias total presentaban un aumento de entre el 30% y 60% del costo del mismo. Esto es un tema crucial a la hora de definir precios óptimos, ya que, es evidente que a mayores precios se obtiene una mayor ganancia unitaria, pero esta medida afectará la cantidad de ventas. Por lo que poder entender que porcentaje de recargo realizar en cada producto es fundamental para maximizar la ganancia de cualquier negocio.
...
`GANANCIAS` = (`PRICE` - `costos`) x `QUANTITY`, y finalmente se seleccionó el precio que maximiza dicha ganancia.
...
scale_fill_continuous(
low = "white", high = "steelblue", name = "Cantidad de clientes por estado"
) +
theme(legend.position = "right")
```
