...
plot_ly(
  ...
  layout(
    ...
    legend = list(title = list(text = "Producto"))
  )   
```
...
```{r}
plot_data_1 <- ds_ganancias %>%
  mutate(
    pct_increase = (PRICE - initial_ticket_price)/initial_ticket_price,
    bin = ntile(pct_increase, 5)  # divide en 5 percentiles
  ) %>%
  group_by(bin) %>%
  summarise(
