...
author: "Franco Santini - Manuel Moresi - Simón Gazze"
date: "`r Sys.Date()`"
output:
...
![](imagen-precios.jpg){width=650px}
...
Se trabajará con información proveniente de las transacciones realizadas en 9 cadenas de negocios minoristas en un período de 3 años, donde se podrá obtener información de cada uno de los negocios y diferentes características relacionadas al producto que se vende. Todo el análisis se realizará usando el Software estadístico `R` y está disponible en este [repositorio](https://github.com/Franco-Santini/Concurso-UBA).
...
Se realizó una inspección por cada una de las variables de interés y en la base de datos `eci_transaction` se encontraron valores faltantes en la variable `QUANTITY`, la cual como se mencionó anteriormente puede calcularse como `TOTAL_SALES`/`PRICE`, de esta manera se pudo reconstruir el dato e imputar todos los valores faltantes de dicha variable.
...
## Objetivo 1 - Estimación de la demanda
...
Se cuenta con información de artículos pertenecientes a 73 subproductos distintos, e inicialmente se quiere evidenciar si hay diferencias en la demanda para los distintos subproductos.
...
plot_ly(
  ...
  layout(
    title = "Top 20 Subgrupos por Demanda total (en USD)",
...
Claramente se observa que hay 9 subproductos que generaron mucha mayor demanda que el resto, estos son *Headphones*, *Speakers*, *Wearables*, *Desktops*, *Smartphones*, *Home Audio*, *Tablets*, *Accesories* y *Laptops*, acumulando un 42% de la demanda total.
...
Cada uno de estos subgrupos se pueden agrupar en 23 grupos distintos, y a su vez estos grupos pertenecen a 8 categorías distintas. A continuación se presentan las distribuciones en relación a la demanda total para estas variables.
...
plot_ly(
  ...
  layout(
    title = list(
      text = "Distribucion de la demanda por subproducto y grupo",
      x = 0.5,
      xanchor = "center"
    )
  )
...
low = "white", high = "#A61C3C", name = "Demanda (en $)"
) +
  theme(legend.position = "right",
        plot.title = element_text(hjust = 0.5, size = 14)) +
  ggtitle("Demanda total según estado")
```
...
scale_fill_continuous(
low = "white", high = "#A61C3C", name = "Demanda (en USD)"
) +
  theme(legend.position = "right",
        plot.title = element_text(hjust = 0.5, size = 14)) +
  ggtitle("Demanda promedio por tienda según estado")
```
...
p1 <- plot_ly(
  ...
  type = "bar",
  name = "Demanda total (USD)",
  marker = list(color = "#A61C3C")
) %>%
  layout(
    xaxis = list(title = "Demanda total (USD)"),
...
subplot(p1, p2, nrows = 2, shareY = TRUE, titleY = TRUE) %>%
  layout(title = "Demanda total (USD) y Cantidad de Tiendas por Tipo",
    xaxis = list(
...
Se puede apreciar con claridad que las tiendas de tipo *Street* y *Mall* son las que mayores demanda presentan, con la particularidad que existen las mismas cantidades de tiendas de estos tipos (44 cada uno), y sin embargo las de tipo *Street* tuvieron una demanda de 48.194.937 $USD mayor.
...
p4 <- plot_ly(
  ...
  x = ~reorder(BRAND, -total_ventas),
  type = "bar",
  name = "Cantidad Tiendas por Cadena",
  marker = list(color = "#DEB887")
) %>%
  layout(
    xaxis = list(title = "Cantidad de Tiendas"),
    yaxis = list(title = "Cadena")
  )

subplot(p3, p4, nrows = 2, shareY = TRUE, titleY = TRUE) %>%
  layout(title = "Demanda total (USD) y Cantidad de Tiendas por Cadena",
    xaxis = list(
...
Se evidencia que la cadena *Essentials Plus* es la que mayor demanda presenta, siendo también la cadena con mayor cantidad de tiendas. 

### Técnicas estadísticas

Antes de comenzar con el ajuste de modelos, se tiene en cuenta que la variable a predecir es cuantitativa, por lo que se opta por utilizar modelos de regresión. En primera instancia se consideraron varias alternativas, como regresión lineal, Random Forest, modelos de Gradient Boosting, series de tiempo, entre otros. Sin embargo, dado el enfoque de predicción de cada uno de ellos, se decide trabajar con Random Forest y Gradient Boosting, para captar relaciones no lineales y predecir la demanda total de manera más eficiente, las variables explicativas consideradas fueron: `PRICE`, `SUBGROUP`, `STORE_TYPE`, `REGION`, `STORE_ID`, `mes`, `dia`, `año`, `category`, `group` y `BRAND`.

Dado que Random Forest posee múltiples hiperparámetros, se lleva a cabo un proceso de optimización de los mismos. Para ello, se divide el conjunto de datos en 85% entrenamiento y 15% prueba. Dentro del 85% destinado a entrenamiento se realiza un proceso de validación cruzada con 4 "folds" con el objetivo de identificar aquellos hiperparámetros que maximizan el $R^2$. Luego, con el 15% restante se prueba el modelo ajustado con los hiperparámetros óptimos, verificando que el $R^2$ obtenido sea consistente con el alcanzado en el proceso de validación. Finalmente, se reentrenará el modelo con los hiperparámetros óptimos con la totalidad de los datos para posteriormente realizar la predicción de la demanda total.
...
Se utilizaron 10950 modelos SARIMA para modelar patrones semanales dado que se intuye que los precios en una semana de cada subgrupo de productos en cada tienda tienden a ser los mismos. Éstos resultan de las combinaciones de las 150 tiendas y 73 subgrupos a predecir, las restantes 150 combinaciones son para las tiendas que venden subgrupos de productos de “Basketball” pero éstas son imputadas al precio que se venden los productos de “Baseball”.
...
La diferencia entre esta opción y la otra es la forma de como se modelan las series temporales, en este caso, se utilizaron 10950 modelos SARIMA para modelar patrones diarios e intentar captar la mayor cantidad de información posible para el pronóstico del precio. Éstos resultan de las combinaciones de las 150 tiendas y 73 subgrupos a predecir, las restantes 150 combinaciones son para las tiendas que venden subgrupos de productos de “Basketball” pero éstas son imputadas al precio que se venden los productos del “Baseball”.
...
-   Random Forest + modelos SARIMA para la determinación del precio semanal: 0.12876

-   Random Forest + modelos SARIMA para la determinación del precio diario: 0.10491

-   Gradient Boosting + modelos SARIMA para la determinación del precio semanal: 0.13456

Por lo tanto, el modelo usado para la predicción de la demanda total en la primer semana de enero del 2024 fue **Gradient Boosting + modelos SARIMA para la determinación del precio semanal**.
...
Se observa que hay un producto que se ha vendido considerablemente más que el resto, el cual es *Fashionista Infants Collections* con un total de 205.692 ventas. Otra cuestión de interés, radica en que todos los productos con mayor cantidad de ventas son productos con precios moderados/bajos, lo que indicaría que no necesariamente se correspondan con los productos que mayor ganancia proporcionaron. El precio promedio de estos productos a través de todas las transacciones fue de 35,95 $USD.
...
plot_ly(
  ...
  layout(
    title = "Ganancias totales (USD) por unidad",
    xaxis = list(title = "Producto",
      tickfont = list(size = 9)),
...
De los productos más vendidos solo *Fashionista Infants Collection* se encuentra en el top de mayor ganancia, lo que lleva a pensar que a pesar de venderse en menor cantidad, algunos de los productos más costosos son los que mayor cantidad de ganancia generan. El precio promedio de los productos que más ganancia generan es de 304.97 $USD, casi 9 veces más caros que los productos más vendidos.

También resulta de interés evidenciar como evolucionó la ganancia que generaron los distintos productos a través del tiempo, en este caso se hace hincapié en los 5 productos que más ganancias generaron y se grafican sus ganancias mensuales durante los 3 años de estudio.
...
plot_usmap(data = plot_data, values = "ganancias", regions = "states") +
  scale_fill_continuous(
    low = "white", high = "#A61C3C", name = "Ganancia (en USD)"
  ) +
  theme(legend.position = "right",
        plot.title = element_text(hjust = 0.5, size = 14)) +
  ggtitle("Ganancias totales según estado")
...
low = "white", high = "#A61C3C", name = "Ganancia (en USD)"
  ...
  theme(legend.position = "right",
        plot.title = element_text(hjust = 0.5, size = 14)) +
  ggtitle("Ganancias promedio por tienda según estado")
```
...
Para optimizar el precio con el objetivo de maximizar la ganancia, se buscó modelar la cantidad vendida en función de distintas variables explicativas: `PRICE`, `STORE_ID`, `SKU`, `category`, `group`, `BRAND` y `REGION`. Dado que los productos de Basketball y Baseball comparten el mismo `SKU`, no fue posible diferenciarlos directamente. Además, no se disponía de información específica sobre las ventas de Basketball. Considerando que ambos productos presentan costos similares y que era razonable suponer una demanda similar, se decidió modelar únicamente los productos de Baseball y asignar el mismo precio a los correspondientes de Basketball.
...
Dado que la variable respuesta es discreta, una alternativa adecuada para modelar los datos es la Regresión Poisson. El modelo se expresa como:

$$g(\lambda) = X^t \beta + \epsilon, \ \ \epsilon \sim N(0, \sigma^2I)$$

- $Y$ es el vector de cantidades vendidas que se distribuye $Poisson(\lambda)$
- $g$ es la función de enlace del modelo que es, $ln(\lambda)$
- $X$ es la matriz de diseño
- $\beta$ es el vector de parámetros
- $\epsilon$ es el vector de errores aleatorios

Entre sus ventajas se destaca la facilidad de interpretación del modelo y su adecuación a datos de conteo. Sin embargo, también presenta limitaciones, como el supuesto de que la media y la varianza son iguales ($E(Y) = Var(Y)$), lo cual no siempre se cumple en la práctica. En esos casos, su aplicación puede llevar a conclusiones incorrectas.

#### Gradient Boosting

Otra alternativa es el Gradient Boosting, un enfoque más flexible que no requiere supuestos tan estrictos como la Regresión Poisson. Este tipo de modelo permite capturar relaciones no lineales y patrones complejos en los datos, lo que le otorga mayor capacidad predictiva frente a modelos lineales generalizados.

### Resultados

#### Regresión Poisson

Tras el entrenamiento del modelo con los datos disponibles, se lo aplicó para estimar el precio óptimo de cada producto en cada tienda. Los resultados obtenidos se muestran en la siguiente tabla.

Dado que el número total de combinaciones posibles entre productos y tiendas asciende a 128.100, y con el objetivo de facilitar la interpretación de los resultados, en este documento se presentan únicamente 1.000 combinaciones seleccionadas a modo ilustrativo.

```{r tabla reg poisson}
# Lectura de los datos
precios_optimizados_reg_poisson <- read.csv("Datos/precios_optimizados_poisson.csv")

precios_optimizados_reg_poisson |>
  select(STORE_ID, SKU, category, group, BRAND, REGION, PRICE) |>
  top_n(n = 1000, PRICE) |> 
  kable(
    digits = 2,
    col.names = c("ID Tiendas", "SKU", "Categoría", "Grupo", "Cadena", "Región", "Precio (USD)"),
    format = "html",
    booktabs = TRUE,
    align = "c" 
  ) |>
  ...
  scroll_box(height = "800px", width = "50em")
```

#### Gradient Boosting

Una vez entrenado el modelo con los datos, se procedió a aplicarlo para estimar el precio óptimo de cada producto en cada tienda. El resultado se resume en la siguiente tabla.

Dado que el número total de combinaciones posibles entre productos y tiendas asciende a 128.100, y con el objetivo de facilitar la interpretación de los resultados, en este documento se presentan únicamente 1.000 combinaciones seleccionadas a modo ilustrativo.

```{r tabla gbm}
# Lectura de los datos
precios_optimizados_gbm <- read.csv("Datos/precios_optimizados_gbm.csv")

precios_optimizados_gbm |>
  select(STORE_ID, SKU, category, group, BRAND, REGION, PRICE) |>
  top_n(n = 1000, PRICE) |> 
  kable(
    digits = 2,
...
Una vez realizada la optimización del precio para cada combinación de producto y tienda con ambas estrategias, es necesario determinar cuál resulta más conveniente. En este caso, se considera más apropiado trabajar con modelos capaces de capturar patrones no lineales, por lo que la alternativa de "Gradient Boosting" representa la opción preferida. No obstante, disponer de los resultados de ambas metodologías aporta mayor solidez al análisis, por lo que los hallazgos obtenidos mediante la "Regresión Poisson" no deberían ser descartados.

# Resultados generales y conclusiones

El presente apartado tiene como objetivo sintetizar los principales hallazgos obtenidos a lo largo de este trabajo, en el cual el enfoque estuvo centrado en dos ejes centrales: por un lado, la predicción de la demanda a partir de las transacciones de los últimos 3 años; y por otro, la optimización de precios con el fin de maximizar la ganancia.

Centrandose en la predicción de la demanda, se han probado distintos modelos que incorporan distintas variables que podrían ayudar a explicar la variable respuesta y mejorar dichas predicciones, pero a su vez quedo en evidencia que se presentaban *limitaciones computacionales* al entrenar los modelos de predicción, debido a que los equipos disponibles no contaban con capacidad de procesamiento elevada. Esto implicó que ciertas exploraciones de parámetros y pruebas de modelos más complejos fueran más lentas o directamente inviables. De contar con recursos en la **nube** (AWS, Google Cloud Platform o Microsoft Azure), sería posible ampliar significativamente el horizonte de experimentación, permitiendo probar más combinaciones de parámetros, generar mayor cantidad de pronósticos y, en última instancia, mejorar la calidad y robustez de las soluciones propuestas.

Luego, a la hora de pensar estrategias para la maximización de las ganancias en base a la fluctuación en el precio de los productos, se pudieron identificar las siguientes cuestiones de interés:

- *Estrategias de ventas*

Como se pudo observar a lo largo de este trabajo, los productos que generaron mayor ganancia en los últimos 3 años corresponden a la categoría *Electronics*. Estos artículos, en general, tienen un valor unitario más elevado y, en consecuencia, una ganancia por unidad superior, a pesar de que la cantidad total de ventas no sea tan alta en comparación con otras categorías.

En este sentido, una recomendación a la empresa sería evaluar la posibilidad de orientar parte de su estrategia de ventas hacia locales especializados en productos electrónicos. Este tipo de productos, al aportar un margen de ganancia más alto, podrían convertirse en un pilar de la rentabilidad futura. A su vez, esto podría estar acompañado en un futuro de una expansión selectiva de locales, priorizando la apertura de tiendas en zonas con alta demanda potencial de productos tecnológicos (grandes centros urbanos, polos universitarios, regiones con mayor ingreso medio) pudiendo maximizar el impacto de la estrategia.

- *Markup*

Como se mencionó a lo largo de este informe, el *markup* representa el porcentaje que se adiciona al costo de un producto para establecer su precio de venta. Definirlo correctamente es fundamental, ya que un *markup* demasiado elevado puede reducir la demanda al alejar a los clientes, mientras que uno demasiado bajo puede comprometer la rentabilidad de la empresa. En este sentido, encontrar un equilibrio resulta esencial para sostener un nivel de ingresos saludable y competitivo.

Si bien los niveles de *markup* pueden variar en función de la industria y del tipo de producto, el análisis realizado con los datos disponibles muestra que un rango comprendido entre el 30% y el 60% ha generado los mayores niveles de ganancia.

Como recomendación se sugiere que la empresa adopte una política de precios flexible dentro de este rango, ajustando el *markup* en función de la elasticidad de la demanda y de las condiciones del mercado. Dentro de ese margen, conviene ajustar el porcentaje según los objetivos del momento: un *markup* más alto aumentará la ganancia por unidad pero puede reducir las ventas, mientras que uno más bajo puede favorecer una mayor rotación de productos y ayudar a sostener o ganar mercado. Lo más conveniente es probar diferentes niveles dentro de este rango para encontrar el equilibrio que maximice la ganancia total sin perder competitividad.

- *Zonas de mayor ganancia*

En base a lo visto anteriormente, el estado que presentó una mayor ganancia promedio por tienda fue el estado de *Nuevo México*, con la particularidad de que en dicho estado solo se encuentran 2 tiendas minoristas. Este resultado sugiere una oportunidad estratégica: **considerar la apertura de nuevos puntos de venta en dicho estado**, con el fin de aprovechar el potencial de rentabilidad que allí se evidencia.

- *Información sobre los clientes*

Al no poder relacionar la información de los clientes con las transacciones realizadas, las posibilidades de análisis se acotan en gran medida. Una solución para poder aprovechar un poco la información disponible en esta base se centra en analizar la geoespacialidad de los clientes, es decir, poder evidenciar en que zonas del país hay mayor cantidad de clientes/socios.

Se visualiza la cantidad de clientes según el estado en el cual residen.
...
scale_fill_continuous(
low = "white", high = "#A61C3C", name = "Cantidad de clientes"
) +
  theme(legend.position = "right",
        plot.title = element_text(hjust = 0.5, size = 14)) +
  ggtitle("Cantidad de clientes según estado")

```

Se observa que los estados del sureste de los Estados Unidos son los que presentan mayor número de clientes, lo que resulta de interés ya que en estos mismos estados se presentó una mayor demanda y una mayor cantidad de ganancias. Las zonas con menor presencia de consumidores representan una oportunidad de crecimiento para la empresa. Una estrategia posible sería fortalecer la difusión en redes sociales y canales digitales, resaltando los beneficios de convertirse en *loyalty members* y generando campañas de publicidad segmentadas que apunten a captar nuevos clientes en dichas regiones.
